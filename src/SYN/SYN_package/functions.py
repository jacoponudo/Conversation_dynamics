...
def calculate_ECDF(df, time_intervals, activate_tqdm=True):
    ...
    for post_id, group_data in grouped:
        ...
        for time in time_intervals:
            ...
            comments_within_time_cc = np.sum(group_data[df['sequential_number_of_comment_by_user_in_thread'] != 1]['temporal_distance_birth_base_100h'] < time)
...
def simulate_number_of_comments(alpha, lambda_, size=1):
    ...
    counts = np.round(np.random.exponential(1 / lambda_, size)).astype(int)
    counts[counts < 0] = 0
        ...
        for i in range(len(target_list)):
            if math.isnan(target_list[i]):
                ...
                if (new_value - sum(target_list[0:i-1])) < iat:
                    target_list[i] = iat[0]
                elif (new_value - sum(target_list[0:i-1])) > 3 * iat:
                    target_list[i] = iat[0]
                else:
